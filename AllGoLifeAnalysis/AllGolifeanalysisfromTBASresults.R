...
AllGoLIFEFULL<-read.csv("ALL-GOLIFE-TBAS.csv", header = TRUE)%>%
  filter(if_any(Likelihood.weight, ~ . >= 0.95))
...
write.csv(AllOTUfilter, "AllGoLifeOTu.csv")
TestNMDS<-read.csv("AllGoLifeOTu_filtered.csv")
sumtable<-function(x){
  if(is.numeric(x)){
    sum(x) > 1
  } else {
...
AllOTUfilter2<-TestNMDS[, sapply(TestNMDS,  sumtable)]
###Import the metadata we will be using and simplify to only include the samples in out OTU tablw
MetaAll<-read.csv("GoLifeAllMetaCorrected.csv", header=TRUE)#%>%
  #rename(Query.sequence = Code)
MetaAllFilter<-merge(MetaAll, AllOTUfilter2, by.x="Query.sequence")
MetaAllFilter<-MetaAllFilter[,1:13]
...
AllOTUTableNMDS<-column_to_rownames(AllOTUfilter2, var="Query.sequence")
...
OTUbySite<-MetaPlusOTU2[,c(4,15:178)] %>%
  group_by(Site)%>%
